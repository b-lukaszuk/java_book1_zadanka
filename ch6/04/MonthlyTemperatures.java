...
public class MonthlyTemperatures {
	...
	for (int week = 1; week <= temperatureIn.length; week++) {
		...
		System.out.println("enter temperature for week " +
				   week + " and day " + day);
    ...
    // method to display all temperatures in teh 2D array requires a nested loop
    static void displayAllTemps(double[][] temperatureIn) {
	System.out.println();
	System.out.println("***TEMPERATURES ENTERED***");
	// the oter loop controls teh week number
	for (int week = 1; week <= temperatureIn.length; week++) {
	    // the inner loop controls the day number
	    for (int day = 1; day <= temperatureIn[0].length; day++) {
	...
	// inpute validation: week number should be between 1 and 4
	while (week < 1 || week > 4)
	    {
